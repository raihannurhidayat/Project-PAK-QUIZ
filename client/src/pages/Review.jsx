...
const Review = ({ answare, index, setDisplay, setIndex }) => {
  ...
  return (
    <div className="h-full relative p-[5px] flex flex-col gap-[20px]">
      <div className="flex text-white text-[32px] justify-center text-center">
        {`Question ${index + 1} : ${questions[index].question}`}
      </div>
      <div className="w-full flex justify-center">
        <div className="h-[161px] text-[32px] w-[500px] rounded-[20px] flex text-white p-5 bg-copper border-solid border-4 border-white">
          {answare[index + 1]
            ? `${answare[index + 1]} . ${
                questions[index].options[answare[index + 1]]
              }`
            : "You didnt answare this question"}
        </div>
      </div>
      <div className="flex items-center justify-center gap-2 text-pastelPink text-[64px]">
        {questions[index].answer === answare[index + 1]
          ? "Correct"
          : "Incorrect"}
        <span>
          {questions[index].answer === answare[index + 1] ? (
      ...
      <div className="text-center text-pastelPink text-[20px]">
        the answare is : {questions[index].options[questions[index].answer]}
      </div>
